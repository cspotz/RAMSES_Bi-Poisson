...
contains
  ...
  subroutine load_grafic
    ...
    do ilevel=levelmin,nlevelmax
       ...
       do idim=1,ndim
             ...
             do i3=1,n3(ilevel)
                ...
                if(active(ilevel)%ngrid>0)then
                   if(i3.ge.i3_min.and.i3.le.i3_max)then
                      init_array(i1_min:i1_max,i2_min:i2_max,i3) = &
                           & init_plane(i1_min:i1_max,i2_min:i2_max)
          !            init_array2(i1_min:i1_max,i2_min:i2_max,i3) = &
        ...
        !     if(myid==1)close(11)
             ...
             if(read_pos) then
                ...
                do i3=1,n3(ilevel)
                   if(myid==1)then
                      ...
                      if(idim==1)read(11)((init_plane_x2(i1,i2),i1=1,n1(ilevel)),i2=1,n2(ilevel))
                   else
                      init_plane_x=0
                      if(idim==1)init_plane_x2=0
                   endif
                   buf_count=n1(ilevel)*n2(ilevel)
#ifndef WITHOUTMPI
                   call MPI_BCAST(init_plane_x,buf_count,MPI_REAL,0,MPI_COMM_WORLD,info)
                   if(idim==1)call MPI_BCAST(init_plane_x2,buf_count,MPI_REAL,0,MPI_COMM_WORLD,info)
#endif
                   if(active(ilevel)%ngrid>0)then
                      if(i3.ge.i3_min.and.i3.le.i3_max)then
                         init_array_x(i1_min:i1_max,i2_min:i2_max,i3) = &
                              & init_plane_x(i1_min:i1_max,i2_min:i2_max)
                         if(idim==1)init_array_x2(i1_min:i1_max,i2_min:i2_max,i3) = &   !BiP mask
                              & init_plane_x2(i1_min:i1_max,i2_min:i2_max)
                      end if
                   endif
                end do
                if(myid==1)close(10)
                if(idim==1.and.myid==1)close(11)
             end if	
             if(read_ids) then
                if(myid==1)then
          ...
          if(active(ilevel)%ngrid>0)then
             ...
             init_array2=dfact(ilevel)*dx/dxini(ilevel)*init_array2/vfact(ilevel)
             if(read_pos)then
			...
			if(init_array_x2(i1,i2,i3)==0)then
                         ipart=ipart+1
						                               !BiP current setup for Dirac-Milne, here only the second species is taken into account as particle as the first one is in the hydro sector
       ...
       if(active(ilevel)%ngrid>0)then
          deallocate(init_array,init_array2,init_array_x)
       end if
       deallocate(init_plane,init_plane2,init_plane_x)
	...
	deallocate(init_array_x2,init_plane_x2) !BiP deallocate mass sign Dirac-Milne
    ...
    do ipart=jpart+1,npart
       ...
       mp(ipart)=0d0
       idp(ipart)=0
       typep(ipart)=int2part(0) !erase type also
    end do
    ...
    ! Setup DM for all particles//Not there for Dark matter
  !  do ipart=1, npart
  !     typep(ipart)%family = FAM_DM
  !     typep(ipart)%tag = 0
  !     if(ipart.gt.npart/2)       typep(ipart)%tag = 1
  !  end do
