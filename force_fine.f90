...
subroutine force_fine(ilevel,icount)
  ...
  rho_m_loc =0; rho_m_all =0
  epot_m_loc=0; epot_m_all=0
  fourpi=2*twopi
...
subroutine gradient_phi_m(ind_grid,ngrid,ilevel,icount)
  ...
  integer,dimension(1:nvector)::ind_grid
  !-------------------------------------------------
  ! This routine compute the minus 3-force for all cells
  ! in grids ind_grid(:) at level ilevel, using a
  ! 5 nodes kernel (5 points FDA).
  !-------------------------------------------------
  integer::i,idim,ind,iskip,nx_loc
  ...
  integer ,dimension(1:nvector,0:twondim),save::igridn
  real(dp),dimension(1:nvector),save::phi1,phi2,phi3,phi4
  real(dp),dimension(1:nvector,1:twotondim,1:ndim),save::phi_m_left,phi_m_right
  ...
  if (ilevel>levelmin)then
     do idim=1,ndim
        call interpol_phi_m(ind_left (1,idim),phi_m_left (1,1,idim),ngrid,ilevel,icount)
        call interpol_phi_m(ind_right(1,idim),phi_m_right(1,1,idim),ngrid,ilevel,icount)
     end do
  end if
  ! Loop over cells
  do ind=1,twotondim
     ...
     do idim=1,ndim
        ...
        do i=1,ngrid
           if(igridn(i,ig1)>0)then
              phi1(i)=phi_m(igridn(i,ig1)+ih1)
           else
              phi1(i)=phi_m_left(i,id1,idim)
           end if
        end do
        do i=1,ngrid
           if(igridn(i,ig2)>0)then
              phi2(i)=phi_m(igridn(i,ig2)+ih2)
           else
              phi2(i)=phi_m_right(i,id2,idim)
           end if
        end do
        do i=1,ngrid
           if(igridn(i,ig3)>0)then
              phi3(i)=phi_m(igridn(i,ig3)+ih3)
           else
              phi3(i)=phi_m_left(i,id3,idim)
           end if
        end do
        do i=1,ngrid
           if(igridn(i,ig4)>0)then
              phi4(i)=phi_m(igridn(i,ig4)+ih4)
           else
              phi4(i)=phi_m_right(i,id4,idim)
           end if
        end do
        do i=1,ngrid
