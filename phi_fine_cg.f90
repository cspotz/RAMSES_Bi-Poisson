...
subroutine phi_fine_cg(ilevel,icount)
  ...
  if(iter >= itermax)then
     if(myid==1)write(*,*)'Positive Poisson failed to converge...'
  end if
...
subroutine phi_m_fine_cg(ilevel,icount)
  ...
  !===============================
   if(ilevel>levelmin)then
      call make_initial_phi_m(ilevel,icount)              ! Interpolate phi_m down
   else
      call make_multipole_phi_m(ilevel)            ! Fill up with simple initial guess
   endif
   call make_virtual_fine_dp(phi_m(1),ilevel)      ! Update boundaries
   call make_boundary_phi(ilevel)                ! Update physical boundaries
  ...
  do while(error>epsilon*error_ini.and.iter<itermax)
     ...
     ! Update boundaries
     call make_virtual_fine_dp(f_m(1,2),ilevel)

     !==============================================
     ! Compute z = Ap and store it into f(i,3)
     !==============================================
     call cmp_Ap_cg_m(ilevel)
     ...
     do ind=1,twotondim
        ...
        do i=1,active(ilevel)%ngrid
           idx=active(ilevel)%igrid(i)+iskip
           phi_m(idx)=phi_m(idx)+alpha_cg*f_m(idx,2)
        end do
     end do
     ...
     do ind=1,twotondim
        ...
        do i=1,active(ilevel)%ngrid
           idx=active(ilevel)%igrid(i)+iskip
           f_m(idx,1)=f_m(idx,1)-alpha_cg*f_m(idx,3)
        end do
     end do
  ...
  if(myid==1)write(*,115)ilevel,iter,error/rhs_norm,error/error_ini
  if(iter >= itermax)then
     if(myid==1)write(*,*)'Negative Poisson failed to converge...'
  end if

  ! Update boundaries
  call make_virtual_fine_dp(phi_m(1),ilevel)

111 format('   Entering phi_m_fine_cg for level ',I2)
112 format('   ==> Step=',i5,' Error=',2(1pe10.3,1x))
115 format('   ==> Level=',i5,' Step=',i5,' Error=',2(1pe10.3,1x))

end subroutine phi_m_fine_cg
!###########################################################
!###########################################################
!###########################################################
!###########################################################
...
subroutine cmp_residual_cg_m(ilevel,icount)
  ...
  do igrid=1,ncache,nvector
     ...
     ! Interpolate potential from upper level
     do idim=1,ndim
        call interpol_phi_m(ind_left (1,idim),phi_left (1,1,idim),ngrid,ilevel,icount)
        call interpol_phi_m(ind_right(1,idim),phi_right(1,1,idim),ngrid,ilevel,icount)
     end do
...
end subroutine cmp_residual_cg_m
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine cmp_Ap_cg_m(ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           ...
           do i=1,ngrid
              if(igridn(i,ig1)>0)then
                 phig(i,idim)=f_m(igridn(i,ig1)+ih1,2)
              else
                 phig(i,idim)=0
              end if
           end do
           ...
           do i=1,ngrid
              if(igridn(i,ig2)>0)then
                 phid(i,idim)=f_m(igridn(i,ig2)+ih2,2)
              else
                 phid(i,idim)=0
              end if
           end do
...
subroutine make_initial_phi_m(ilevel,icount)
  ...
  do igrid=1,ncache,nvector
     ...
     if(ilevel==1)then
        ...
        ! Interpolate
        call interpol_phi_m(ind_cell_father,phi_int,ngrid,ilevel,icount)
        ...
        do ind=1,twotondim
           ...
           do i=1,ngrid
              phi_m(ind_cell(i))=phi_int(i,ind)
           end do
           do idim=1,ndim
              do i=1,ngrid
                 f_m(ind_cell(i),idim)=0 !BiP
...
end subroutine make_initial_phi_m
!###########################################################
!###########################################################
!###########################################################
!###########################################################
...
end subroutine make_multipole_phi_m
